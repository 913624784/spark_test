...
object UserNetAx {
  ...
  def transformDataList(line: String) = {
    ...
    if (beginTime.getDay != endTime.getDay || (endTime.getHours >= 23 && endTime.getMinutes >= 30)) {
      stayUpLateCount += 1
    }
    //熬夜时长
  ...
  def main(args: Array[String]): Unit = {
    ...
    val file = sc.textFile("D://file/washedNetDataFile.csv")
    ...
    //将分析后的数据写入文件
    for (i <- lines) {
      WriteFile.writeInfo("D://file/userNetAnalysisData.csv", i + "\n")
    }
